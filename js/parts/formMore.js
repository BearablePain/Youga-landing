function formMore() {
  ...
  statusMessage.classList.add('status'); //Создаем обработчик на форму 
  ...
  form.addEventListener('submit', event => {
    ...
    event.preventDefault(); //оповещаем пользователя о статусе

    form.appendChild(statusMessage); //создаем объект(конструктор) для запросов

    let request = new XMLHttpRequest(); //cсоздаем мктод для настройки нашего AJAX запроса 
    ...
    request.setRequestHeader('Content-type', 'application/json', 'charset=utf-8'); //получаем данные которые ввел пользователь
    ...
    let formData = new FormData(form); //проебразовываем в JSONфайл
    //Создаем промежуточный объект

    let obj = {}; // записываем наши ключи и значение полученные от пользователя в объект
    ...
    let json = JSON.stringify(obj); //открываем(отправляем на сервер) наш запрос(те данные что ввел пользователь)
