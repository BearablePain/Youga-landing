function createTimer() {
  ...
  function setClock(id, endTime) {
    ...
    seconds = timer.querySelector('.seconds'),
        //секунды
    days = timer.querySelector('.days'); //делаем таймер динамичным с помощью сетинтервала

    let timeInterval = setInterval(updateClock, 1000); //Записываем значения
    ...
    function updateClock() {
      ...
      days.textContent = t.days; //Добавляем ноль, когда секунд,минут,часов меньше дясяти
      ...
      if (t.total <= 0) {
        //останавливаем интервал по завершению
