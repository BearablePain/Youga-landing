function createSlider() {
  let slideIndex = 1,
      //Указываем параметр текущего слайда 
  slides = document.querySelectorAll('.slider-item'),
      prev = document.querySelector('.prev'),
      next = document.querySelector('.next'),
      dotsWrap = document.querySelector('.slider-dots'),
      dots = document.querySelectorAll('.dot'); // функция проверяет, скрывает, и поазывает активность
  ...
  function showSlides(n) {
    ...
    slides.forEach(item => item.style.display = 'none'); //делаем точки неактивными

    dots.forEach(item => item.classList.remove('dot-active')); //показываем первый слайд и точку
