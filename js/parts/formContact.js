function formContact() {
  ...
  statusMessageContact.classList.add('status'); //Создаем функцию дял отправки форму 

  function sendForm(elem) {
    elem.addEventListener('submit', event => {
      event.preventDefault();
      elem.appendChild(statusMessageContact);
      let contactFormData = new FormData(elem); //внутри создаем функци.
      ...
      function postData(data) {
        ...
        return new Promise((resolve, reject) => {
          ...
          requestContact.setRequestHeader('Content-type', 'application/json', 'charset-utf-8'); //преобразуем в JSON
      ...
      postData(contactFormData).then(() => statusMessageContact.innerHTML = messageContact.loading).then(() => statusMessageContact.innerHTML = messageContact.sucess).catch(() => statusMessageContact.innerHTML = messageContact.error).then(clearInput);
    });
  }
...
module.exports = formContact; //Вариант без промисов
